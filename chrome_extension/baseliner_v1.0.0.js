(function(){
	...
	if (myBody.className.indexOf('baseliner') > 0) {
		...
		window.baselinerUpdateTop = function(topChange){
				baseTop = topChange;
				removeRules();
				addRules(baseBg, baseTop);
				console.log('%c Baseliner is now starting at ' + parseInt(topChange) + '. ', 'background: #DFDFDF; color: #209C39');
			};
		//topChange.addEventListener('input', baselinerUpdateTop);
		...
		window.baselinerUpdatesImage = function (baselineChange){
			var canvas = document.createElement('canvas'),
				context = canvas.getContext('2d'),
				height = baselineChange;
			canvas.width = 4;
		...
		//baselineChange.addEventListener('input', baselinerUpdatesImage);
